...
@app.get("/upcoming_tasks/{user_id}")
async def get_upcoming_tasks(user_id: int):
    ...
    if conn:
        ...
        cur.execute("""
            SELECT id, time, task, completed FROM upcoming_tasks WHERE user_id = %s ORDER BY time ASC;
        """, (user_id,))
        ...
        if not tasks:
            return {"message": "No upcoming tasks found."}

        return [{"id": task[0], "time": task[1], "task": task[2], "completed": task[3]} for task in tasks]
